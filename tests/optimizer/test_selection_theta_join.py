...
class TestSelectionThetaJoinRule(unittest.TestCase):
    ...
    def test_is_applicable_valid(self):
        join = QueryTree(type="THETA_JOIN", value="A.id = B.id", children=[])
        sel = QueryTree(type="SELECTION", value="A.age > 30", children=[join])

        self.assertTrue(self.rule.is_applicable(sel))

    def test_is_applicable_invalid_type(self):
        node = QueryTree(type="JOIN", value=None, children=[])
        self.assertFalse(self.rule.is_applicable(node))

    def test_is_applicable_invalid_child(self):
        child = QueryTree(type="TABLE", value="A", children=[])
        node = QueryTree(type="SELECTION", value="x > 0", children=[child])
        self.assertFalse(self.rule.is_applicable(node))

    def test_apply_merge_conditions(self):
        left = QueryTree(type="TABLE", value="A", children=[])
        right = QueryTree(type="TABLE", value="B", children=[])
        join = QueryTree(type="THETA_JOIN", value="A.id = B.id", children=[left, right])
        sel = QueryTree(type="SELECTION", value="A.age > 30", children=[join])

        new_node = self.rule.apply(sel)

        self.assertIsNotNone(new_node)
        self.assertEqual(new_node.type, "THETA_JOIN")
    ...
    def test_apply_when_only_selection_condition(self):
        ...
        left = QueryTree(type="TABLE", value="A", children=[])
        right = QueryTree(type="TABLE", value="B", children=[])
        join = QueryTree(type="THETA_JOIN", value="", children=[left, right])
        sel = QueryTree(type="SELECTION", value="A.age > 30", children=[join])
    ...
    def test_apply_not_applicable(self):
        table = QueryTree(type="TABLE", value="A", children=[])
        sel = QueryTree(type="SELECTION", value="A.age > 30", children=[table])
